...
dev: ## Run parser limited by 300 records
	...
	@bin/parser --conn="root:dthcnf07@/mitdb?parseTime=true&loc=Local" --stdout=./logs/stdout.log --limit=1 --search=true

test-id: ## Run single page parser by id
	@echo 'Starting parser...'
	@echo '' > ./logs/stdout.log
	@bin/parser --conn="root:dthcnf07@/mitdb?parseTime=true&loc=Local" --stdout=./logs/stdout.log --test.id=148633 --search=true

prod: ## Run parser without limit
	@echo 'Starting parser...'
	@echo '' > ./logs/stdout.log
	@bin/parser --conn="root:dthcnf07@/mitdb?parseTime=true&loc=Local" --stdout=./logs/stdout.log --search=true
