...
type User struct {
	...
	Options       null.JSON   `boil:"options" json:"options,omitempty" toml:"options" yaml:"options,omitempty"`
...
var (
	userColumns               = []string{"id", "role_id", "name", "email", "password", "remember_token", "options", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"role_id", "name", "email", "password", "remember_token", "options", "created_at", "updated_at"}
...
func (o *User) Offers(mods ...qm.QueryMod) offerQuery {
...
func (o *User) Pricelists(mods ...qm.QueryMod) pricelistQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`pricelists`.`user_id`=?", o.ID),
	)
...
func (userL) LoadOffers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`offers`), qm.WhereIn(`user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadPricelists(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`pricelists`), qm.WhereIn(`user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *User) AddOffers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Offer) error {
	var err error
...
// Sets o.R.User's Offers accordingly.
// Replaces o.R.Offers with related.
// Sets related.R.User's Offers accordingly.
func (o *User) SetOffers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Offer) error {
	query := "update `offers` set `user_id` = null where `user_id` = ?"
...
// Sets related.R.User.
func (o *User) RemoveOffers(ctx context.Context, exec boil.ContextExecutor, related ...*Offer) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.UserID, nil)
		if rel.R != nil {
...
// Sets related.R.User appropriately.
func (o *User) AddPricelists(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Pricelist) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
