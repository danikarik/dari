...
func testCustomerToManyOffers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadOffers(ctx, tx, false, (*[]*Customer)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testCustomerToManyAddOpOffers(t *testing.T) {
	var err error
...
func testCustomerToManySetOpOffers(t *testing.T) {
	var err error
...
func testCustomerToManyRemoveOpOffers(t *testing.T) {
	var err error
...
var (
	customerDBTypes = map[string]string{`Address`: `text`, `BankDetails`: `text`, `CreatedAt`: `timestamp`, `Email`: `varchar`, `ID`: `int`, `Iin`: `varchar`, `Name`: `varchar`, `UpdatedAt`: `timestamp`}
	_               = bytes.MinRead
)
