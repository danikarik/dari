...
func testRegistryJournalToOneUserUsingUser(t *testing.T) {
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, registryJournalDBTypes, true, registryJournalColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize RegistryJournal struct: %s", err)
	}
	...
	if err = local.L.LoadUser(ctx, tx, false, (*[]*RegistryJournal)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
...
func testRegistryJournalToOneRemoveOpUserUsingUser(t *testing.T) {
	var err error
