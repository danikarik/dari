...
func (o *Category) Discounts(mods ...qm.QueryMod) discountQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`discounts`.`category_id`=?", o.ID),
	)
...
// Subcategories retrieves all the subcategory's Subcategories with an executor.
func (o *Category) Subcategories(mods ...qm.QueryMod) subcategoryQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`subcategories`.`category_id`=?", o.ID),
	)
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"`subcategories`.*"})
	}
...
func (categoryL) LoadDiscounts(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCategory interface{}, mods queries.Applicator) error {
	var slice []*Category
	var object *Category
	...
	query := NewQuery(qm.From(`discounts`), qm.WhereIn(`category_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (categoryL) LoadProducts(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCategory interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`products`), qm.WhereIn(`category_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadSubcategories allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (categoryL) LoadSubcategories(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCategory interface{}, mods queries.Applicator) error {
	var slice []*Category
	var object *Category
	...
	query := NewQuery(qm.From(`subcategories`), qm.WhereIn(`category_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load subcategories")
	}

	var resultSlice []*Subcategory
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice subcategories")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on subcategories")
	}
	if err = results.Err(); err != nil {
	...
	if singular {
		object.R.Subcategories = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.CategoryID {
				local.R.Subcategories = append(local.R.Subcategories, foreign)
				if foreign.R == nil {
					foreign.R = &subcategoryR{}
				}
				foreign.R.Category = local
...
func (o *Category) AddDiscounts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Discount) error {
	var err error
...
// AddSubcategories adds the given related objects to the existing relationships
// of the category, optionally inserting them as new records.
// Appends related to o.R.Subcategories.
// Sets related.R.Category appropriately.
func (o *Category) AddSubcategories(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Subcategory) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.CategoryID = o.ID
	...
	if o.R == nil {
		...
		o.R.Subcategories = append(o.R.Subcategories, related...)
	}
...
func (o *Category) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for categories")
	}
