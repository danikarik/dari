...
func testCountryToManyManufacturers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadManufacturers(ctx, tx, false, (*[]*Country)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testCountryToManyAddOpManufacturers(t *testing.T) {
	var err error
...
var (
	countryDBTypes = map[string]string{`Code`: `varchar`, `ID`: `int`, `IsoCode2`: `varchar`, `IsoCode3`: `varchar`, `Name`: `varchar`}
	_              = bytes.MinRead
)
