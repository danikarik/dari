...
func testCategoryToManyDiscounts(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadDiscounts(ctx, tx, false, (*[]*Category)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testCategoryToManySubcategories(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, subcategoryDBTypes, false, subcategoryColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, subcategoryDBTypes, false, subcategoryColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	subcategory, err := a.Subcategories().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := CategorySlice{&a}
	if err = a.L.LoadSubcategories(ctx, tx, false, (*[]*Category)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Subcategories); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Subcategories = nil
	if err = a.L.LoadSubcategories(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Subcategories); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	...
	if t.Failed() {
		t.Logf("%#v", subcategory)
	}
}

func testCategoryToManyAddOpDiscounts(t *testing.T) {
	var err error
...
func testCategoryToManyAddOpSubcategories(t *testing.T) {
	var err error
	...
	var a Category
	var b, c, d, e Subcategory

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, categoryDBTypes, false, strmangle.SetComplement(categoryPrimaryKeyColumns, categoryColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*Subcategory{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, subcategoryDBTypes, false, strmangle.SetComplement(subcategoryPrimaryKeyColumns, subcategoryColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Subcategory{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddSubcategories(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Subcategories[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Subcategories[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Subcategories().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	categoryDBTypes = map[string]string{`CreatedAt`: `timestamp`, `ID`: `int`, `Name`: `varchar`, `UpdatedAt`: `timestamp`}
	_               = bytes.MinRead
)
